 ...
 * @author zhanglinfeng
 */
public class MyDefaultTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices, ConsumerTokenServices, InitializingBean {


    private static final Logger log = LoggerFactory.getLogger(MyDefaultTokenServices.class);
    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        OAuth2AccessToken existingAccessToken = getOAuth2AccessTokenByAuthentication(authentication);
     ...
     * @return
     */
    private OAuth2AccessToken getOAuth2AccessTokenByAuthentication(OAuth2Authentication authentication) {
        OAuth2AccessToken existingAccessToken = tokenStore.getAccessToken(authentication);
        //允许多个终端登陆，则正常返回token信息
        if (!singleClientToken) {
            return existingAccessToken;
        }
        if (existingAccessToken != null) {
            //存在有效的accessToken ,直接删除即可
            if (existingAccessToken.getRefreshToken() != null) {
        ...
        if (tokenStore instanceof RedisTokenStore) {
            Collection<OAuth2RefreshToken> tokens = ((RedisTokenStore) tokenStore).findRefreshTokensByClientIdAndUserName(clientId, userName);
            if (!CollectionUtils.isEmpty(tokens)) {
                log.info("当前clientId:[{}],userName:[{}],获取到的已经存在的token为:[{}]", tokens, clientId, userName);
                tokens.forEach(oAuth2RefreshToken -> tokenStore.removeRefreshToken(oAuth2RefreshToken));
            }
        }
        return null;
    }
    ...
    public OAuth2AccessToken refreshAccessToken(String refreshTokenValue, TokenRequest tokenRequest)
        ...
        if (this.authenticationManager != null && !authentication.isClientOnly()) {
            //如果当前鉴权类型为自定义的对象，并且为没有用户信息，则不执行后续的代码
            boolean isAuthUser = authentication.getPrincipal() instanceof AuthUser;
            boolean needAuthenticate = !isAuthUser || !((AuthUser) authentication.getPrincipal()).getNoUserInfo();
            if (needAuthenticate) {
