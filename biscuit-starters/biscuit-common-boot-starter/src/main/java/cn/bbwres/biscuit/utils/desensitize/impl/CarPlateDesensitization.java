...
/**
 * 车牌号脱敏：
 * 保留前2位（省份+字母），后3位用*代替（如京A****）
 *
 * @author zhanglinfeng
 */
public class CarPlateDesensitization implements DefaultDesensitization {
    /**
     * 脱敏实现
     *
     * @param target 脱敏对象
    ...
    public String desensitize(String target, int prefixLen, int suffixLen) {
        return DesensitizationUtils.desensitizeCustom(target, prefixLen == -1 ? 2 : prefixLen, suffixLen == -1 ? 0 : suffixLen);
    }
}
