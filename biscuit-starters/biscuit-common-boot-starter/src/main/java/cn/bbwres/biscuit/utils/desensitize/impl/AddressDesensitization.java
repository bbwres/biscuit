...
/**
 * 地址脱敏
 * 地址脱敏：保留前6位（省市区），后续用*代替（如北京市海淀区****）
 *
 * @author zhanglinfeng
 */
public class AddressDesensitization implements DefaultDesensitization {
    /**
     * 脱敏实现
     *
     * @param target 脱敏对象
    ...
    public String desensitize(String target, int prefixLen, int suffixLen) {
        return DesensitizationUtils.desensitizeCustom(target, prefixLen == -1 ? 6 : prefixLen, suffixLen == -1 ? 0 : suffixLen);
    }
}
